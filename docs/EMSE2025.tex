...
To evaluate FairReweighing on a larger real-world image-derived regression task, we use SCUT-FBP5500 with 5,500 face images and attractiveness ratings. The primary SCUT experiments use fixed image-derived features or embeddings as regressors' inputs, with facial beauty rating as $Y$ and sex/race inferred from the dataset metadata or filenames as sensitive attributes. This design keeps FairReweighing as the object of study rather than conflating it with end-to-end representation learning. We also report neural-network results as supplementary context. Table~\ref{tab:SCUT_avg} and Table~\ref{tab:SCUT_R2} summarize results when training and testing with average ratings from 60 raters and ratings from the P8 rater. When the untreated model already has low separation violation, we do not interpret additional reductions as strong evidence; instead, SCUT primarily tests scalability, runtime, and behavior under a larger non-tabular feature distribution.
...
FairReweighing is most useful when the desired fairness notion is separation and the practitioner wants a model-agnostic mitigation step. Because the method operates by assigning sample weights, it can be used before training linear models, support-vector methods, tree ensembles, and neural predictors that accept sample weights. It is especially relevant for regression datasets where $Y$ or $A$ is continuous and ordinary frequency-count Reweighing is not well defined.
